...
export async function GET(req: Request) {
  ...
  for (const r of rows || []) {
    const node: DriveNode = {
      ...
      ownerName: (r as any).owner_name ?? null,
      ownerEmail: (r as any).owner_email ?? null,
      size: (r as any).size ?? null,
      modifiedAt: (r as any).modified_time ?? null,
      createdAt: (r as any).created_time ?? null,
      videoDurationMs: (r as any).video_duration_ms ?? null,
      videoWidth: (r as any).video_width ?? null,
      videoHeight: (r as any).video_height ?? null,
      status: "Synced",
      subRows: [],
    }
